...
def main() -> None:
    """
    Skapar bredare level2 corridor check-KML för svårare routingfall.
    ...
    for row in rows:
        ...
        if link_id in traversal_ids:
            color = "ff0000ff"
            width = 5
            name = f"Wide traversal link {link_id}"
        else:
            color = "66ffffff"
            width = 1
            name = f"Wide local candidate link {link_id}"
        placemarks.append(
            f"""
    <Placemark>
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>Level2 Corridor Check Wide</name>{''.join(placemarks)}
  </Document>
</kml>
"""
    output = Path("examples") / "level2_corridor_check_wide.kml"
    output.write_text(content, encoding="utf-8")
    print(output)
    print(f"Kandidatlänkar {len(candidate_ids)}")
