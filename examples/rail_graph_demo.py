...
def main() -> None:
    ...
    repaired_graph = RailGraph.from_config_file()
    repaired_graph.build(repair_missing_nodes=True, snap_tolerance_m=25.0)
    ...
    print("Reparerad graf", repaired_graph.summary())
    ...
    print("Reparerad komponentstorlek punkt1", repaired_graph.connected_component_size(start_node_oid))
    print("Reparerad komponentstorlek punkt2", repaired_graph.connected_component_size(end_node_oid))

    try:
        path = graph.shortest_path(start_node_oid, end_node_oid)
        print("Grafväg", f"{len(path.link_ids)} länkar", f"{path.total_length_m:.2f} m", f"kostnad {path.total_cost:.2f}")
    except Exception as exc:
        print("Ingen grafväg hittad", type(exc).__name__, str(exc))

    try:
        repaired_path = repaired_graph.shortest_path(start_node_oid, end_node_oid)
    except Exception as exc:
        print("Ingen reparerad grafväg hittad", type(exc).__name__, str(exc))
        return

    print("Reparerad väg", f"{len(repaired_path.link_ids)} länkar", f"{repaired_path.total_length_m:.2f} m", f"kostnad {repaired_path.total_cost:.2f}")
    print("Reparerade länkar", repaired_path.link_ids)
